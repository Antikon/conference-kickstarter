$(document).ready(function() {
    ...
    $('.parsable-document').on('change filebatchselected', function (e) {
        $(this).trigger('blur');

        $('.document-not-parsed').hide();
    ...
    $('#no-grants').on('change', function (e) {

        var checked = $(this).prop('checked');

        var $grants = $('#grants-input');

        if (checked) {
            $grants.val('').prop('disabled', true);
            $grants.blur();
        } else {
            $grants.prop('disabled', false);
            $grants.blur();
        }
...
*/
    ...
    function displayError() {
        $('.document-not-parsed').show();
    }

/*
    if ($('#contributors-id-en').val() === '') {
        $('.generate-english-contributors').trigger('click');
    }
